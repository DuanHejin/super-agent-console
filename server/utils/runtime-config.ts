export function getAppRuntimeConfig() {
    ...
    return {
        ...
        modelProvider: readRuntimeString(
            'MODEL_PROVIDER',
        ...
        modelApiKey:
            readRuntimeString('MODEL_API_KEY', config.modelApiKey) || undefined,
        modelEnabled: readRuntimeBoolean(
            'MODEL_ENABLED',
            config.modelEnabled,
            true,
        ),
        modelTemperature: readRuntimeNumber(
            'MODEL_TEMPERATURE',
            config.modelTemperature,
            0.2,
        ),
        modelTopP: readRuntimeNumber('MODEL_TOP_P', config.modelTopP, 0.8),
        modelMaxTokens: readRuntimeNumber(
            'MODEL_MAX_TOKENS',
            config.modelMaxTokens,
            4096,
        ),
        demoServerToken: readRuntimeString(
            'DEMO_SERVER_TOKEN',
            config.demoServerToken,
        ),
        accessCodes: readRuntimeString('ACCESS_CODES', config.accessCodes),
        adminAccessCodes: readRuntimeString(
            'ADMIN_ACCESS_CODES',
            config.adminAccessCodes,
        ),
        authCookieSecret: readRuntimeString(
            'AUTH_COOKIE_SECRET',
            config.authCookieSecret,
        ),
        runRateLimitMinute: readRuntimeNumber(
            'RUN_RATE_LIMIT_MINUTE',
            config.runRateLimitMinute,
            3,
        ),
        runRateLimitDay: readRuntimeNumber(
            'RUN_RATE_LIMIT_DAY',
            config.runRateLimitDay,
            30,
        ),
        globalRunRateLimitMinute: readRuntimeNumber(
            'GLOBAL_RUN_RATE_LIMIT_MINUTE',
            config.globalRunRateLimitMinute,
            5,
        ),
        globalRunRateLimitDay: readRuntimeNumber(
            'GLOBAL_RUN_RATE_LIMIT_DAY',
            config.globalRunRateLimitDay,
            100,
        ),
        concurrentRunsPerUser: readRuntimeNumber(
            'CONCURRENT_RUNS_PER_USER',
            config.concurrentRunsPerUser,
            1,
        ),
        concurrentRunsGlobal: readRuntimeNumber(
            'CONCURRENT_RUNS_GLOBAL',
            config.concurrentRunsGlobal,
            3,
        ),
        authLoginRateLimitMinute: readRuntimeNumber(
            'AUTH_LOGIN_RATE_LIMIT_MINUTE',
            config.authLoginRateLimitMinute,
            10,
        ),
        ...
        maxRunInputLength: readRuntimeNumber(
            ...
            config.maxRunInputLength,
            5000,
        ),
        logLevel: readRuntimeString('LOG_LEVEL', config.logLevel, 'info'),
        mockModelEnabled: readRuntimeBoolean(
            'MOCK_MODEL_ENABLED',
            config.mockModelEnabled,
            true,
        ),
    };
}
...
function readRuntimeString(
    ...
    return (
        process.env[envKey] ||
        process.env[`NUXT_${envKey}`] ||
        String(configValue || fallback)
    );
}

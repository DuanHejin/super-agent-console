...
- [ ] 新增创建消息接口：`POST /api/conversations/messages`。
- [ ] 在创建消息接口中实现 `clientRequestId` 幂等校验，重复请求返回同一组 `conversationId` / `messageId` / `runId`。
- [ ] 新增 SSE 订阅接口：`GET /api/agent/runs/:runId/events`。
- [ ] 将前端发送流程改为：先创建消息和 Run，再用 `runId` 建立 SSE 订阅。
...
- [ ] 同一个 `clientRequestId` 连续请求两次，不重复创建 conversation / message / run。
- [ ] 前端可以通过 `runId` 订阅一次 Agent Run 的执行过程。
